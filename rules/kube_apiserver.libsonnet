    ...
    groups+: [
      {
        ...
        rules: [
        ...
        ] + [
          {
            ...
            expr: |||
              sum(rate(apiserver_request_duration_seconds_sum{subresource!="log",verb!~"LIST|WATCH|WATCHLIST|DELETECOLLECTION|PROXY|CONNECT"}[5m])) by (%(clusterLabel)s)
              /
              sum(rate(apiserver_request_duration_seconds_count{subresource!="log",verb!~"LIST|WATCH|WATCHLIST|DELETECOLLECTION|PROXY|CONNECT"}[5m])) by (%(clusterLabel)s)
            ||| % ($._config),
          },
        ] + [
          {
